...
(prove:subtest "keymap->map"
  (let ((keymap (keymap:make-keymap))
    ...
    (prove:is (keymap:keymap->map keymap)
              (fset:map ("a" 'foo-a)
                        ("b" 'foo-b)
                        ("k a" 'bar-a)
                        ("k c" 'bar-c))
              :test #'fset:equal?)
    (prove:is (keymap:keymap->map keymap keymap2)
              (fset:map ("a" 'foo-a)
                        ("b" 'foo-b)
                        ("c" 'bar-c)
                        ("k a" 'bar-a)
                        ("k c" 'bar-c))
              :test #'fset:equal?)
    (prove:is (keymap:keymap->map keymap2 keymap)
              (fset:map ("a" 'bar-a)
                        ("b" 'foo-b)
                        ("c" 'bar-c)
                        ("k a" 'bar-a)
...
(prove:subtest "keymap-with-parents->map"
  (let* ((grand-parent (keymap:make-keymap))
    ...
    (prove:is (keymap:keymap-with-parents->map keymap)
              (fset:map ("a" 'foo-a)
                        ("b" 'bar-b)
                        ("c" 'qux-c)
                        ("d" 'quux-d))
...
(prove:subtest "keymap-with-parents->map with cycles" ; TODO: Can we check warnings?
  (let ((keymap1 (keymap:make-keymap))
    ...
    (prove:is (keymap:keymap-with-parents->map keymap1)
              (fset:empty-map)
              :test #'fset:equal?))
  (let ((keymap1 (keymap:make-keymap))
        (keymap2 (keymap:make-keymap))
