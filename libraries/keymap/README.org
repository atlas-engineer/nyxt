...
- A =key= structure has a hardware =code=, a symbolic =value=, an ordered set of
  ...
  Whitespaces are the only reserved characters.  Anything else can be store in
  the =key= value.

- A =keyspec= is a string representation of a =key=.  A =keyspecs= is a
  space-separate string representation of a sequence of =key='s.

- A =keymap= is contains a hash-table where the keys are a keyspec and the values
