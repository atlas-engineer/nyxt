...
(defun submatches (suggestion source input) ; TODO add some tests?
...
`delete-inexact-matches' preprocessor."
  (let ((terms (remove-duplicates (str:split-omit-nulls " " input)
                                  :test #'string-equal)))
    (when (funcall
           (apply #'alex:conjoin
                  (mapcar (lambda (term)
                            (lambda (suggestion-match-data)
                              (str:contains? term suggestion-match-data
                                             :ignore-case t)))
                          terms))
