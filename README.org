...
In most cases, it should already be running for you.

If Next does not start up however, it is very likely that D-Bus is not running
for your user.  You can start D-Bus manually manually with the following shell
command:
