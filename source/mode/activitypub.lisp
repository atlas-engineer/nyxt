...
(defgeneric send-object (object)
  ...
  (:method ((object base))
    (alex:if-let ((serialization (unparse-object object))
                  (profile (profile (find-submode (current-buffer) 'nyxt/activitypub-mode:activitypub-mode))))
      (multiple-value-bind (content code headers)
          (dex:post (url* (outbox profile))
