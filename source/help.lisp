...
(define-command describe-any ()
  ...
  (prompt
   ...
   :sources (list (make-instance 'variable-source
                                 :actions (list (make-unmapped-command describe-variable)))
                  (make-instance 'function-source
                                 :actions (list (make-unmapped-command describe-function)))
                  (make-instance 'user-command-source
                                 :actions (list (make-unmapped-command describe-command)))
                  (make-instance 'class-source
                                 :actions (list (make-unmapped-command describe-class)))
                  (make-instance 'slot-source
                                 :actions (list (make-unmapped-command describe-slot))))))
...
(define-internal-page-command describe-variable
    (&optional (variable-suggestion
                (first (prompt
                        :prompt "Describe variable:"
                        :sources (make-instance 'variable-source)))))
    (buffer (str:concat "*Help-" (symbol-name variable-suggestion) "*")
            'nyxt/help-mode:help-mode)
  "Inspect a variable and show it in a help buffer."
  (let ((help-mode (find-mode buffer 'help-mode)))
    (setf (nyxt/help-mode:inspected-value help-mode) variable-suggestion)
    (spinneret:with-html-string
      (:style (style buffer))
      (:h1 (format nil "~s" variable-suggestion)) ; Use FORMAT to keep package prefix.
      (:pre (documentation variable-suggestion 'variable))
      (:h2 "Current Value:")
      (:p (:raw (value->html (symbol-value variable-suggestion) :help-mode help-mode))))))

(define-internal-page-command describe-function
    (&optional (function-suggestion (first (prompt
                                            :prompt "Describe function"
                                            :sources (make-instance 'function-source)))))
    (buffer (str:concat "*Help-" (symbol-name function-suggestion) "*")
            'nyxt/help-mode:help-mode)
...
For generic functions, describe all the methods."
  (flet ((method-desc (method buffer)
           (spinneret:with-html-string
             (:style (style buffer))
             (:h1 (symbol-name function-suggestion) " " (write-to-string (mopu:method-specializers method)))
             (:pre (documentation method 't))
             (:h2 "Argument list")
             (:p (write-to-string (closer-mop:method-lambda-list method))))))
    (if (typep (symbol-function function-suggestion) 'generic-function)
        (apply #'str:concat (mapcar (alex:rcurry #'method-desc buffer)
                                    (mopu:generic-function-methods
                                     (symbol-function function-suggestion))))
        (str:concat
         (spinneret:with-html-string
           (:style (style buffer))
           (:h1 (format nil "~s" function-suggestion) ; Use FORMAT to keep package prefix.
                (when (macro-function function-suggestion) " (macro)"))
           (:pre (documentation function-suggestion 'function))
           (:h2 "Argument list")
           (:p (write-to-string (mopu:function-arglist function-suggestion))))
         #+sbcl
         (unless (macro-function function-suggestion)
           (spinneret:with-html-string
             (:h2 "Type")
             (:p (format nil "~s" (sb-introspect:function-type function-suggestion)))))))))

(define-internal-page-command describe-command
    (&optional (command (first (prompt
...
(define-internal-page-command describe-class
    (&optional (class-suggestion (first (prompt
                                         :prompt "Describe class"
                                         :sources (make-instance 'class-source)))))
  (buffer (str:concat "*Help-" (symbol-name class-suggestion) "*")
          'nyxt/help-mode:help-mode)
  "Inspect a class and show it in a help buffer."
  (let* ((slots (class-public-slots class-suggestion))
         (slot-descs (apply #'str:concat (mapcar (alex:rcurry #'describe-slot* class-suggestion) slots))))
    (spinneret:with-html-string
      (:style (style buffer))
      (:h1 (symbol-name class-suggestion))
      (:p (:pre (documentation class-suggestion 'type)))
      (when (mopu:direct-superclasses class-suggestion)
        (:h2 "Direct superclasses:")
        (:ul (loop for class-name in (mapcar #'class-name (mopu:direct-superclasses class-suggestion))
                   collect (:li (:a :href (lisp-url `(nyxt::%describe-class ',class-name)) class-name)))))
      (when (mopu:direct-subclasses class-suggestion)
        (:h2 "Direct subclasses:")
        (:ul (loop for class-name in (mapcar #'class-name (mopu:direct-subclasses class-suggestion))
                   collect (:li (:a :href (lisp-url `(nyxt::%describe-class ',class-name)) class-name)))))
      (:h2 "Slots:")
      (:raw slot-descs)
      (:h2 "Methods:")
      (:ul (loop for method in (remove-if
                                #'listp (mapcar #'mopu:generic-function-name
                                                (mopu:generic-functions class-suggestion)))
    ...
    (spinneret:with-html-string
      ...
      (:ul
       (loop for mode in modes
             collect (:li (:a :href
                              (lisp-url
                               `(nyxt::%describe-class ',(sera:class-name-of mode)))
      ...
      (:ul
       (loop for source in sources
             collect (:li (:a :href
                              (lisp-url
                               `(nyxt::%describe-class ',(sera:class-name-of source)))
