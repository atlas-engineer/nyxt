...
;; TODO: Macro to automate this process
(serapeum:export-always 'ffi-window-delete)
(defmethod ffi-window-delete ((window window))
  (declare (ignore window))
  (error 'unsupported-operation))
(defmethod ffi-window-fullscreen ((window window))
  (declare (ignore window))
  (error 'unsupported-operation))
(defmethod ffi-window-unfullscreen ((window window))
  (declare (ignore window))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-window-make)
(defmethod ffi-window-make ((browser browser))
  (declare (ignore browser))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-window-to-foreground)
(defmethod ffi-window-to-foreground ((window window))
  (declare (ignore window))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-window-set-title)
(defmethod ffi-window-set-title ((window window) title)
  (declare (ignore window title))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-window-active)
(defmethod ffi-window-active ((browser browser))
  (declare (ignore browser))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-window-set-active-buffer)
(defmethod ffi-window-set-active-buffer ((window window) (buffer buffer))
  (declare (ignore window buffer))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-window-set-minibuffer-height)
(defmethod ffi-window-set-minibuffer-height ((window window) height)
  (declare (ignore window height))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-make)
(defmethod ffi-buffer-make ((browser browser) &key title default-modes)
  (declare (ignore browser title default-modes))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-delete)
(defmethod ffi-buffer-delete ((buffer buffer))
  (declare (ignore buffer))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-load)
(defmethod ffi-buffer-load ((buffer buffer) uri)
  (declare (ignore buffer uri))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-evaluate-javascript)
(defmethod ffi-buffer-evaluate-javascript ((buffer buffer) javascript &key callback)
  (declare (ignore buffer javascript callback))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-minibuffer-evaluate-javascript)
(defmethod ffi-minibuffer-evaluate-javascript ((window window) javascript &key callback)
  (declare (ignore window javascript callback))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-enable-javascript)
(defmethod ffi-buffer-enable-javascript ((buffer buffer) value)
  (declare (ignore buffer value))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-enable-javascript-markup)
(defmethod ffi-buffer-enable-javascript-markup ((buffer buffer) value)
  (declare (ignore buffer value))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-set-proxy)
(defmethod ffi-buffer-set-proxy ((buffer buffer) &optional proxy-uri (ignore-hosts (list nil)))
  (declare (ignore buffer proxy-uri ignore-hosts))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-buffer-get-proxy)
(defmethod ffi-buffer-get-proxy ((buffer buffer))
  (declare (ignore buffer))
  (error 'unsupported-operation))
(serapeum:export-always 'ffi-generate-input-event)
(defmethod ffi-generate-input-event ((window window) event)
