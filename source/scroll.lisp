...
(define-parenscript %scroll-down (&key (scroll-distance (scroll-distance (current-buffer))))
  (ps:chain window (scroll-by 0 (ps:lisp scroll-distance))))

(define-parenscript %scroll-up (&key (scroll-distance (scroll-distance (current-buffer))))
  (ps:chain window (scroll-by 0 (ps:lisp (- scroll-distance)))))

(define-parenscript %scroll-left (&key (horizontal-scroll-distance
                                        (horizontal-scroll-distance (current-buffer))))
  (ps:chain window (scroll-by (ps:lisp (- horizontal-scroll-distance)) 0)))

(define-parenscript %scroll-right (&key (horizontal-scroll-distance
                                         (horizontal-scroll-distance (current-buffer))))
  (ps:chain window (scroll-by (ps:lisp horizontal-scroll-distance) 0)))
