...
(defmethod ffi-buffer-download ((buffer gtk-buffer) url)
  (let* ((webkit-download (webkit:webkit-web-view-download-uri (gtk-object buffer) url))
    ...
    (connect-signal download "decide-destination" nil (webkit-download suggested-file-name)
      (alex:when-let* ((download-dir (downloads-directory buffer))
                       (downloads-directory (nfiles:expand download-dir))
                       (native-download-directory (unless (nfiles:nil-pathname-p downloads-directory)
                                                    (uiop:native-namestring downloads-directory)))
