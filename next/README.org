...
** OSX Instructions
*** Installing CCL
- Download [[https://ccl.clozure.com/download.html][CCL]], full installation instructions can be found [[https://ccl.clozure.com/install.html][here]].
...
2. Copy the script located at ~ccl/scripts/ccl64~ somewhere into your
   executable path
3. You may want to make an alias or rename ~ccl64~ to ~ccl~
** Linux Instructions
...
1. ~ccl -l load~
2. This will have loaded nEXT, to launch nEXT, execute
   ~(next:start)~. A fully functional window should appear.
3. The repl will remain accessible via the terminal.
