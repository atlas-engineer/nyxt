...
~libwebkit2gtk-3.0.so~ (the shared library for WebKit2GTK+).

For Ubuntu this package is ~libwebkit2gtk-3.0-dev~
For Debian this package is ~libwebkit2gtk~

For now, nEXT only supports ~libwebkit2gtk-3.0.so~ due to
compatibility issues with cl-webkit.
