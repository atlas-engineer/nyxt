...
* Send current URL to KDEConnect devices
  ...
  #+end_src
    ...
    The following command lists available devices with =-a= option. If none are already detected, it probes them with =-l= option, and lists available devices again. After parsing the device id from selected string, it executes the command to send the url.
  ...
  #+begin_src lisp
    (define-command kdeconnect-send-url (&optional (url (url (current-buffer)))
      ...
      (let ((device-strings
              (trivia:match (kdeconnect-devices kdeconnect-command "-a")
