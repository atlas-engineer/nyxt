...
**** Non-Quicklisp Setup
If you aren't using Quicklisp, you can configure Nyxt to start a Slynk server with the following steps.
...
2. Add the following lines to your Nyxt's init.lisp (the configuration file) depending on whether you are using the Emacs Sly package, or you cloned the repository:
   ...
   - *If you are pointing Nyxt to your Emacs Sly package installation directory add these lines to your =nyxt/init.lisp=:*
   #+NAME: nyxt/init.lisp
   #+begin_src lisp
   ;; Change the push directory below to where you've cloned Sly.
   (asdf:load-system :slynk)
   (slynk:create-server :port 4008)
   #+end_src

   - *If you are cloning from git, it's the same as above but with an extra =push= expression:*
     Remember to replace the directory in the push expression with where you cloned Sly.
     #+NAME: nyxt/init.lisp
     #+begin_src lisp
