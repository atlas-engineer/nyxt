...
Nyxt repository into =~/.local/share/common-lisp/systems=, so that ASDF finds
it.

(This path is looked up non-recursively, thus the Common Lisp libraries checked
out as Git-submodules in Nyxt won't be mistakenly seen by ASDF.)
...
Otherwise clone each repository into =~/.local/share/common-lisp/systems= (or
[[https://www.common-lisp.net/project/asdf/asdf.html#Configuring-ASDF-to-find-your-systems][another directory where ASDF -will find it]]).  For instance:

#+begin_src sh
  mkdir -p ~/.local/share/common-lisp/systems
  git clone https://github.com/joachifm/cl-webkit ~/.local/share/common-lisp/systems/cl-webkit
#+end_src
...
cl-webengine repository into =~/.local/share/common-lisp/systems= (or [[https://www.common-lisp.net/project/asdf/asdf.html#Configuring-ASDF-to-find-your-systems][another
directory where ASDF -will find it]]). For instance:

#+begin_src sh
  mkdir -p ~/.local/share/common-lisp/systems
  git clone git@github.com:atlas-engineer/cl-webengine.git ~/.local/share/common-lisp/systems/cl-webengine
#+end_src
...
Clone the Nyxt repository into =~/.local/share/common-lisp/systems=:

#+begin_src sh
mkdir -p ~/.local/share/common-lisp/systems
git clone https://github.com/atlas-engineer/nyxt ~/.local/share/common-lisp/systems/nyxt
#+end_src
