...
In order to start this interaction of glory and hacks, run the command
=start-swank= in a compiled version of Nyxt.

Notice that the default values of the variable ~*swank-port*~ in Nyxt is
different from its counterpart variable =slime-port= in Emacs to avoid
collisions with an ~*inferior-lisp*~ process.

After launching the Swank server in Nyxt, complete the following steps
in Emacs:

1. Make sure you have =SLIME= installed in Emacs
2. ~M-x slime-connect~
3. Enter ~127.0.0.1~ for the host
4. Enter the port number set in the Nyxt variable ~*swank-port*~
   (e.g. ~4006~)

To customize the port that Swank starts on, edit the global variable
~slime-port~ in your Emacs init file.
...
SLIM and ILISP.  If you already know SLIME, SLY's closeness to is
...
First, follow the following steps in Nyxt:

1. Install =cl-slynk= (available on Guix)
2. Add the following line to your Nyxt init file (usually located at
   =~/.config/nyxt/init.lisp=)
  #+begin_src lisp
    (load-after-system :slynk "~/.config/nyxt/my-slink.lisp")
  #+end_src
3. Create a file called =my-slink.lisp= where you will specify your
   =start-slynk= command
  #+NAME: my-slink.lisp
  #+begin_src lisp
  ...
  #+end_src
4. Run the command =start-lynk= in Nyxt

Then, in Emacs:

1. Make sure you have =SLY= installed in Emacs
2. ~M-x sly-connect~
3. Enter ~127.0.0.1~ for the host
4. Enter the port number set in the Nyxt variable ~*swank-port*~
   (e.g. ~4006~)
