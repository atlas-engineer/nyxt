...
up-to-date or else Nyxt might fail to build.  In a Lisp REPL:
...
Nyxt requires some Lisp libraries.  Some pinned at specific versions, some are
forks, so you should not rely on Quicklisp.

All Lisp dependencies are included as a submodule of this repository, so if you
did not disable submodules (with ~NYXT_SUBMODULES=false~), there is nothing more
to be done.
...
*Steps:*
1. Add the following lines to your Nyxt init file. If your Quicklisp directory
...
2. Add the following lines to your Nyxt's init.lisp (the configuration file)
   ...
   - If you have installed Sly with =package-install= (or similar method) in
     Emacs, add these lines to your =nyxt/init.lisp= :
...
2. Create a file called =my-slynk.lisp= in your Nyxt configuration directory
   (see =(nyxt-init-file)= where you will create a =start-slynk= command.
