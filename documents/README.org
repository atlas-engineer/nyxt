...
- We suffix preficatess with =-p=.  Unlike the usual convention, we always use a
  =-= even for single words.
- Prefer the term =url= over =uri=.
- All URLs should have the =quri:uri= type.  If you need to manipulate a URL
  string, call it =url-string=.
- Paths should have the =cl:pathname= type.
