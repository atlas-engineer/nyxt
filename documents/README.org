...
- feature branches (named after the features they add) :: for most pull requests;
...
the complexity of the diff.  Most feature branches should use =master= as the
target branch to avoid branch dependence.
