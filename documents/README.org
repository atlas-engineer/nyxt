...
#+begin_src lisp
(load-after-system :slynk "/home/user/.config/nyxt/my-slink.lisp")
#+end_src

After you've added the delcaration above, create a file called
=my-slink.lisp= where you will specify your =start-slynk= command.
...
Use like for =SLIME=, substituting =start-slynk= for =start-swank= in
Nyxt, and =sly-connect= for =slime-connect= in Emacs.
