...
- [[#basics][Basics]]
  ...
  - [[#tls-certificate-host-whitelisting][TLS certificate host whitelisting]]
...
** TLS certificate host whitelisting

By default the WebKit engine refuses to establish a TLS communication to a host with an erroneous
TLS certificate (e.g. self-signed ones). The buffer mode =tls-certificate-whitelist=
allows to mitigate this problem by providing a mechanism to specify a list of hosts
for which TLS certificate errors shall be ignored. A hostname does not contain a protocol
...
(setf next/tls-certificate-whitelist-mode:*default-tls-certificate-whitelist*
      (make-instance 'tls-certificate-whitelist :whitelist '("atlas.engineer")))

(defclass my-buffer (buffer)
  ((default-modes :initform '(web-mode root-mode proxy-mode tls-certificate-whitelist-mode))))
(setf buffer-class 'my-buffer)
#+end_src
