...
To customize  it, you can  write a new function  that takes an  url as
parameter, and bind it  to =next/video:*download-function*=.  In doing
so,  you can  rely on  the =next/video:download=  function, that  does
error handling and process management.

See also  the variable  =*preferred-download-directories*= (a  list of
directories).
