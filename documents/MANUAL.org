...
Note that the above code snippet supposes that your Next was built with dependencies from Quicklisp. If that's not the case (e.g. under Guix), use an appropriate substitute.
