...
- New =nosave= mode.
  A buffer in =nosave= mode does not persist any data to disk.
  (Also knowns as "icognito" or "private" mode in some other browsers.)
...
- New [[https://wiki.gnome.org/Projects/GObjectIntrospection][GObject Introspected]]-based renderer.
