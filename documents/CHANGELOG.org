...
- Rename project to Nyxt.
  This name is more "searchable" on the Web.  It's also reasonable
...
- Ensure that all commands belong to their appropriate namespace.  In
  particular, all webview-related commands now belong to =next/web-mode= (zooming,
  searching, hinting, etc.) and minibuffer commands belong to =next/minibuffer-mode=.
