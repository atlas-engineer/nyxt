...
- New "panel" buffers to display arbitrary content beside the web view.

  - =show-bookmarks-panel=
  - =show-buffers-panel=
  - =delete-panel-buffers=

- New =go-next= and =go-previous= commands.  (Thanks to @pdelfino!)

- New =show-qrcode-of-current-url= command.  (Thanks to @ag91!)

- New VI state indicators.  (Thanks to @edgar-vincent!)

  Both the status area and the prompt buffer now display a colored
  indicator for =vi-normal-mode= and =vi-insert-mode=.

- Input keys are no longer forwarded in =vi-normal-mode=.

  Concretely, pressing an unbound letter won't insert it in an HTML input.

- Support buffer tree relationships.

  - =switch-buffer-previous= and =switch-buffer-next= browse the buffer
    tree in a depth-first fashion.

  - New =switch-buffer-last= command to easily switch back and forth
    between the 2 most recent buffers.

  - =list-buffers= can now display the the buffers as a tree.

- Macro editor: interactively record command macros with =edit-macro=.
  Macros can be saved to the =auto-config.lisp= file.

- =execute-extended-command= prefills default values when provided and
  performs type checking.

- New =edit-user-file-with-external-editor= command.

- New =window= =titler= slot to customize the window title.

- New DOM parser.

  Much of the former Parenscript has been replaced by pure Lisp code.

- Improved hint detection and handling such as with drop-down buttons.

- Password commands now default input to current host.

- =describe-variable= can now describe compound values recursively, such
  as list of variables.

- Allow local commands in key bindings.

  You can create a local command with =make-command=.

- Various manual and tutorial improvements and fixes.  (Thanks to @pdelfino!)

- Deprecate =paste-or-set-url=.

- Deprecated commands no longer show in =execute-command=.

- Replaced =copy-system-information= with =show-system-information=.
...
- Bug fixes:
  ...
  - Start up errors due to flawed user init files now lead to a restart
    of Nyxt without init file.

  - Fix endless compilation of =tutorial.lisp= when SBCL dynamic space
    size to low.
...
- New =expedition-mode=
  ...
  Expedition mode stores a set of links that you can then traverse
  forwards and backwards as if you are on an expedition through the
  Internet!

- New =select-frame-*=

  Select frame allows the user to drag-select multiple elements on
  screen. For example, the user can drag-select to open up all of the
  links of a search result.
...
*** DONE Add no-script-mode to disable JavaScript
